...
namespace EmeraldAI.Utility
{
    public class EmeraldAIProjectile : MonoBehaviour
    {
        ...
        void OnTriggerEnter(Collider C)
        {
            ...
            if (!Collided && EmeraldSystem != null && ProjectileCurrentTarget != null && C.transform.IsChildOf(ProjectileCurrentTarget.transform) && !C.transform.IsChildOf(EmeraldSystem.transform) && C.gameObject.layer != 2)
            {
                if (EffectOnCollisionRef == EffectOnCollision.Yes)
                {
                    if (CollisionEffect != null)
